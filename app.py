...
client = MongoClient('mongodb+srv://test:sparta@cluster0.buavm.mongodb.net/Cluster0?retryWrites=true&w=majority',
                     tlsCAFile=ca)
db = client.dbsparta
...
def boxcheck():
    ...
    result = list(db.posts.find({'gu': {"$regex": checkval_receive}}, {'_id': False}))
    if checkval_receive == "all":
        mini_list = list(db.posts.find({}, {'_id': False}))
